...
pci_2021.fixed_project <- st_as_sf(gwr.fixed_project$SDF) %>%
...
```{r}
# Converting SDF into sf data.frame
pci_2021.fixed_capital <- st_as_sf(gwr.fixed_capital$SDF) %>%
  st_transform(crs=3405)


gwr.fixed.output_capital <- as.data.frame(gwr.fixed_capital$SDF)
pci_2021.fixed_capital <- cbind(pci_2021, as.matrix(gwr.fixed.output_capital))

glimpse(pci_2021.fixed_capital)
```
...
pci_2021.fixed_capital$Local_R2 <- unlist(pci_2021.fixed_capital$Local_R2)


tm_shape(provincial_boundaries)+
  tm_polygons(alpha = 0.1) +
tm_shape(pci_2021.fixed_capital) +  
  tm_polygons(col = "Local_R2",
          border.col = "gray60",
          border.lwd = 1) +
  tm_view(set.zoom.limits = c(5,8))
...
![](images/photo_2024-11-09_12-28-01.jpg)
...
![](images/photo_2024-11-09_12-38-42.jpg)
...
![](images/photo_2024-11-09_12-41-34.jpg)
...
![](images/photo_2024-11-09_12-44-01.jpg)
...
![](images/photo_2024-11-09_12-54-28.jpg)

<br>

### 6.2.2 Visualise Local R2

**Visualizing Coefficient Estimates** in Geographically Weighted Regression (GWR) allows users to observe how the influence of each predictor varies spatially across the study area. This detailed visualization highlights the localized impacts of predictors, uncovering spatial trends and variations that a global model might overlook.

The user selection is the same as Visualising Local R2.

This customizable visualization provides a spatial breakdown of each predictor’s effect, allowing users to see where variables have stronger or weaker influences. By adjusting bandwidth, optimization approach, and kernel, users can refine the analysis, exploring how these factors affect the spatial distribution of predictor impacts and enhancing the understanding of location-specific dynamics in the data.

![](images/photo_2024-11-09_13-01-15.jpg)
